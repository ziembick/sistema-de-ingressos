...
app.get('/api/events/:eventId/spots', async (req, res) => {
  ...
  return res.json({ event, spots: eventSpots });
});
...
app.post('/api/checkout', async (req, res) => {
  ...
  if (!card_hash || !ticket_kind || !spotsName || !email) {
  ...
  for (const spotName of spotsName) {
    ...
    if (spotIndex === -1) {
      return res.status(400).json({ message: `Spot ${spot.id} not found` });
    }
    if (eventSpots[spotIndex].status !== 'available') {
      return res.status(400).json({ message: `Spot ${eventSpots[spotIndex].id} is not available` });
    }
